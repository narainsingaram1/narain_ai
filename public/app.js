...
const state = { areas:[], classes:[], items:[], view:'today', area:null, filter:'open', editing:null, entryType:'task', editingClass:null, calendarMode:'month', selectedDay:dayKey(new Date()), planDay:dayKey(new Date()), movingEventId:null,collapsedTaskGroups:new Set() };
...
async function refresh(){const data=await api('/api/state');state.areas=data.areas;state.classes=data.classes;state.items=data.items;render();if($('#editor').open)renderClassOptions($('#entry-class').value);if($('#classes-dialog').open)renderClasses()}
...
function assistantView(){return `<div class="hero"><div><div class="eyebrow">CONTEXT, ON DEMAND</div><h1 class="page-title">Ask Orbit<span style="color:#a797f6">.</span></h1><p class="muted">Answers grounded in the entries you have saved here.</p></div></div><div class="assistant-layout"><section class="card ask-card"><div class="card-head"><h2>Your workspace assistant</h2><span class="badge">Local answers</span></div><div id="chat-log" class="chat-log"><div class="chat-bubble answer">Hi. Ask what needs attention, what is due this week, or about a topic in your notes. I’ll point back to your saved records.</div></div><form id="ask-form" class="ask-form"><input name="question" maxlength="500" required placeholder="What should I focus on today?" aria-label="Ask Orbit"><button class="primary-button">Ask →</button></form></section><section class="card"><div class="card-head"><h2>Try asking</h2></div><div class="suggestions"><button data-ask="What should I focus on today?">What should I focus on today?</button><button data-ask="What is due this week?">What is due this week?</button><button data-ask="What are my active goals?">What are my active goals?</button><button data-ask="What do I have about career?">What do I have about career?</button></div><p class="muted" style="margin-top:20px">Orbit currently retrieves and ranks your own records. It does not invent plans or connect to a language model yet.</p></section></div>`}
...
function appendChat(kind,message,sources=[]){const box=$('#chat-log');if(!box)return;const bubble=document.createElement('div');bubble.className=`chat-bubble ${kind}`;bubble.textContent=message;if(sources.length){const links=document.createElement('div');links.className='chat-sources';for(const source of sources){const button=document.createElement('button');button.textContent=source.title;button.dataset.edit=source.id;links.append(button)}bubble.append(links)}box.append(bubble);box.scrollTop=box.scrollHeight}
async function ask(question){appendChat('user',question);const input=$('#ask-form input');input.value='';try{const result=await api('/api/assistant',{method:'POST',body:JSON.stringify({question})});appendChat('answer',result.answer,result.sources)}catch(err){appendChat('answer',err.message)}}
